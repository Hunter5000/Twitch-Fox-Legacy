    ...
    for (var key in online_streamers) {
        ...
        checkChannel(function(response) {
            ...
            if (stream == null) {
                ...
                var viewers = stream.viewers.toString()
                var preview = stream.preview
                var avatar = stream.channel.logo
                var namekey = online_streamers.indexOf(strname)
                if ((game != online_games[namekey]) || (title != online_titles[namekey]) || (preview != online_previews[namekey]) || (avatar != online_avatars[namekey])) {
                    //Something has changed... time to update
                    //But let's not update if the viewer number has changed. That can get quite obnoxious with a lot of online streamers
...
function panelUpdate() {
    ...
    panel.port.emit("updatePage", [online_streamers, online_games, online_titles, online_viewers, online_previews, online_avatars, offline_streamers, alarmOn, followed_streamers]);
}
