...
panel.port.on("openLive", function(payload) {
    go("http://www.twitch.tv/" + payload, preferences.liveQuality)
})
...
function panelUpdate() {
    ...
    panel.port.emit("updatePage", [online_streamers, online_games, online_titles, online_viewers, online_avatars, offline_streamers, alarmOn, followed_streamers, preferences.defaultHide, preferences.defaultHide2, preferences.sortingMethod, preferences.enableLivestreamer, preferences.openPopout]);
}
...
function onLiveChange() {
    if (preferences.enableLivestreamer == "A") {
        preferences.liveQuality.hidden = true
    } else {
        preferences.liveQuality.hidden = false
    }
}

require("sdk/simple-prefs").on("importClick", function() {
    var importer = preferences.import_user
...
require("sdk/simple-prefs").on("addremClick", function() {
    var changed = preferences.addremName
    changed = changed.replace(/ /g, "")
    if (changed != "") {
        changed = changed.toLowerCase()
        if (containsValue(followed_streamers, changed)) {
            var nameindex = followed_streamers.indexOf(changed)
            var followed_streamers2 = (preferences.streamer_list).split(",")
            preferences.streamer_list = ""
            var followed_streamers3 = []
            for (var key in followed_streamers2) {
                var newname = followed_streamers2[key].replace(/ /g, "");
                followed_streamers3.push(newname.toLowerCase())
            }
            followed_streamers3.splice(nameindex, 1)
            for (var key in followed_streamers3) {
                if (followed_streamers3[key] != "") {
                    preferences.streamer_list = preferences.streamer_list + ", " + followed_streamers3[key]
                }
            }
            if (preferences.streamer_list.slice(0, 2) == ", ") {
                preferences.streamer_list = preferences.streamer_list.slice(2)
            ...
            followed_streamers3.unshift(changed)
            for (var key in followed_streamers3) {
                if (followed_streamers3[key] != "") {
                    preferences.streamer_list = preferences.streamer_list + ", " + followed_streamers3[key]
                }
